...
cloud_auth <- function(json_path, lifetime_seconds = 3600L) {
  ...
  pk <- openssl::read_key(charToRaw(key$private_key))
...
#' @export
cloud_submit_batch <- function(auth, project, region, bucket, job_id,
                               machine_type, task_type,
                               image = Sys.getenv("CRAIBM_CLOUD_IMAGE", unset = ""),
                               worker_service_account = auth$email,
                               max_run_seconds = 86400L) {

  image <- trimws(as.character(image))
  if (!nzchar(image)) {
    stop("Enter the public GHCR container image before submitting the job.")
  }

  worker_service_account <- trimws(as.character(worker_service_account))
  if (!nzchar(worker_service_account) ||
      !grepl("^[^@[:space:]]+@[^@[:space:]]+$", worker_service_account)) {
    stop("A valid Batch worker service-account email is required.")
  }
